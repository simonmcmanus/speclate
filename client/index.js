...
var loadRequiredFiles = async (requiredLists) => {
  ...
  requiredLists.mappers.forEach(async (mapper) => {
    window.speclate.lists.mappers[mapper] = await import('/lists/mappers/' + mapper + '.js').default;    
  });
  requiredLists.filters.forEach(async (filter) => {
    window.speclate.lists.filters[filter] = await import('/lists/filters/' + filter + '.js').default;    
  });
  requiredLists.lists.forEach(async (list) => {
    window.speclate.lists.lists[list] = await import('/lists/' + list + '.js').default;    
  });
};
